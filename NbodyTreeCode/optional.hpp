...
#define TL_TRAITS_MUTEX
		...
		using enable_if_t = std::enable_if_t<E, T>;
		...
		struct conjunction<B, Bs...>
			: std::conditional_t<static_cast<bool>(B::value), conjunction<Bs...>, B>
		{
		};
...
#define TL_TRAITS_LIBCXX_MEM_FN_WORKAROUND
...
#endif
		          typename = enable_if_t<std::is_member_pointer_v<decay_t<Fn>>>,
		          int = 0>
		...
		struct is_swappable<T[N], T[N]>
			: std::integral_constant<
				...
				(!decltype(
						swap_adl_tests::uses_std<T[N], T[N]>(0))::value ||
		...
		struct is_nothrow_swappable
			: std::integral_constant<
				...
				((decltype(swap_adl_tests::uses_std<T, U>(0))::value
					...
					(!decltype(swap_adl_tests::uses_std<T, U>(0))::value &&
						swap_adl_tests::is_adl_swap_noexcept<T,
						                                     U>::value))>
		{
		};
#endif
		...
		enable_if_t<std::is_constructible_v<T, U&&> &&
			!std::is_same_v<decay_t<U>, in_place_t> &&
			!std::is_same_v<optional<T>, decay_t<U>>>;
	                 ...
	                 private detail::optional_delete_assign_base<T>
	{
		...
		static_assert(!std::is_same_v<detail::decay_t<T>, nullopt_t>,
  ...
  /// object if there is one.
    ...
    !defined(TL_OPTIONAL_GCC54) && !defined(TL_OPTIONAL_GCC55)
		...
		constexpr explicit optional(
			detail::enable_if_t<std::is_constructible_v<T, Args...>, in_place_t>,
			Args&&... args)
		...
		TL_OPTIONAL_11_CONSTEXPR explicit optional(
			detail::enable_if_t<std::is_constructible_v<T, std::initializer_list<U>&,
			                                            Args&&...>,
			                    in_place_t>,
		...
		template <
			...
			detail::enable_if_t<std::is_convertible_v<const U&, T>>* = nullptr>
		...
		template <class U, detail::enable_from_other<T, U, const U&>* = nullptr,
		          detail::enable_if_t<!std::is_convertible_v<const U&, T>>* =
			          nullptr>
		...
		/// valueless.
		void
		swap(optional& rhs) noexcept(std::is_nothrow_move_constructible_v<T>
	...
	template <class T,
	          detail::enable_if_t<std::is_move_constructible_v<T>>* = nullptr,
	...
	template <class T = detail::i_am_secret, class U,
	          class Ret =
	          detail::conditional_t<std::is_same_v<T, detail::i_am_secret>,
	                                detail::decay_t<U>, T>>
	...
	namespace detail
	{
		...
		template <class Opt, class F,
		          class Ret = decltype(detail::invoke(std::declval<F>(),
		                                              *std::declval<Opt>())),
		          enable_if_t<!std::is_void_v<Ret>>* = nullptr>
		...
		optional<monostate> optional_map_impl(Opt&& opt, F&& f)
		{
			if (opt.has_value())
			{
				detail::invoke(std::forward<F>(f), *std::forward<Opt>(opt));
...
namespace std
{
	...
	{
		size_t operator()(const tl::optional<T>& o) const
		{
			if (!o.has_value())
				return 0;
